...
<p>
Load_npz and save_npz consume quite a bit of temp space (and take a long time); see <a href="https://github.com/kwchurch/JSALT_Better_Together/blob/main/src/JSALT_util.py">here</a> for an alternative that uses more space on disk, but loads faster with less temp space.
...
<li>Chebyshev Iterations (10 steps): <a href="https://github.com/kwchurch/JSALT_Better_Together/blob/main/src/ProNE_chebyshev.py">$JSALTsrc/ProNE_chebyshev.py</a></li>
...
and to study space, time and loss.
<ol>
...
bottleneck is the call to SVD in the first step (prefactorization).
