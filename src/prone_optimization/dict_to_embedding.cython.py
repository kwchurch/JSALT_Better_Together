...
def method2model():
    ...
    if m == 'GGVec':
        ...
        #return prone.ProNE(n_components=K)
...
def slow_output_embedding(G, E, outf):                     
    with open(outf, 'w') as fd:
        print(' '.join(map(str, E.shape)), file=fd)
        for w,i in zip(G.nodes(),range(E.shape[0])):
            print(str(w) + ' ' + ' '.join(map(str, E[i,:])), file=fd)
