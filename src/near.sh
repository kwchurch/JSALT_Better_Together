...
cut -f1-3  | sort -nr -u | sed 20q > $spectertmp
...
cut -f1-3  | sort -nr -u | sed 20q > $proposedtmp
...
cat $proposedtmp $proposedtmp | cut -f2- | $JSALTsrc/C/pairs_to_cos  --floats $proposed/embedding.K280.f --record_size 280 --map $proposed/map > $bothtmp.2
