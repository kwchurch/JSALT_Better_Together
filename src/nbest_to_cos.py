...
print('nbest_to_cos: ' + str(sys.argv), file=sys.stderr)
...
for line in sys.stdin:
    ...
    i1 = int(id1)
    ids = np.array([int(x) for x in id2.split('|')], dtype=np.int32)
    d = cosine_similarity(get_vec([i1]), get_vec(ids))
    m1 = imap[i1]
    m2 = imap[ids]
    for mm2, dd in zip(m2, d.reshape(-1)):
        print('%f\t%d\t%d' % (dd, m1, mm2))
