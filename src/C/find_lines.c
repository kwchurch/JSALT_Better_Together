...
void usage()
{
  fatal("usage: find_lines --print_offset [--maxline 1024] [--multiple_matches] [--header] --input lines --fields xxx < keys, where each line starts with key (will create index if necessary in lines.L)\n");
}
...
int _idx_compare(const void *a, const void *b)
{
  return idx_compare((struct idx *)a, (struct idx *)b);
}
...
void create_idx(char *outfile, char *infile)
{
  ...
  qsort(idx, nidx, sizeof(struct idx), _idx_compare);
...
char *lookup(char *buf,char *piece, char *file, long nfile, struct idx *idx, long nidx)
{
  ...
  if(sscanf(piece, "%ld", &query.key) == 1)
    found = bsearch(&query, idx, nidx, sizeof(struct idx), _idx_compare);
...
int main(int ac, char **av)
{
  ...
  char *filename = NULL;
  char *fields = NULL;
  int header=0, multiple_matches=0, print_offset=0;
  int my_maxline = 1024;
  
  for(i=1; i<ac; i++)
    if(strcmp(av[i], "--input") == 0) filename=av[++i];
    else if(strcmp(av[i], "--print_offset") == 0) print_offset++;
    ...
    else if(strcmp(av[i], "--maxline") == 0) my_maxline=atoi(av[++i]);
    else usage();
  ...
  char *file = (char *)mmapfile(filename, &nfile);
  char *end = file + nfile;

  buf = (char *)malloc(my_maxline);
  ...
  if(fields == NULL)
    while(scanf("%ld", &query.key) == 1) {
      struct idx *found = bsearch(&query, idx, nidx, sizeof(struct idx), _idx_compare);
