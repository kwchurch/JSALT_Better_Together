...
int head_compare(struct head *a, struct head *b)
{
  ...
  if(a->eof) return 1;
  if(b->eof) return -1;
  return lbigram_compare(&(a->bigram), &(b->bigram));
}

void advance(struct head *h)
{
  for(;;) {
    if(fread(&(h->bigram), sizeof(struct lbigram), 1, h->fd) != 1) {
      h->eof=1;
      return;
    }
    if(h->bigram.val != 0)
...
void merge_sort(int ac, char **av)
{
  ...
  for(;;) {
    ...
    if(h->eof) return;
    if(fwrite(&(h->bigram), sizeof(struct lbigram), 1, stdout) != 1)
      fatal("write failed");
    advance(h);
  }
}
...
int main(int ac, char **av)
{
  ...
  if(ac != 2) usage();
  long N;
  struct lbigrams *b = (struct lbigrams *)readchars(av[1], &N);
