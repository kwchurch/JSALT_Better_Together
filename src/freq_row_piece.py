...
parser.add_argument("-P", "--piece", help=".npz file", required=True)
...
#     newM = M[indexes,:]
#     scipy.sparse.save_npz('%s.pieces%03d' % (args.output, p), newM)
...
X2s = []
Y2s = []
for i,v in enumerate(Xuniq):

    assert v < M.shape[0], 'cannot find v = %d in M' % v
    ...
    if i>0 and i%args.report == 0:
        t1 = time.time() - t0a
        print('i = %d of %d, sec = %f, i per sec = %f, ETA = %f' % (i, nXuniq, t1, i/t1, (nXuniq * t1/i - t1)), file=sys.stderr)
        sys.stderr.flush()

X = np.concatenate(X2s)
Y = np.concatenate(Y2s)
V = np.ones(len(X), dtype=bool)
Sketch = scipy.sparse.csr_matrix((V, (X, Y)), dtype=bool, shape=M.shape)

outf = args.piece
if outf.endswith('.npz'): outf = outf[0:-4]
scipy.sparse.save_npz(outf + '.sketch', Sketch)
