...
def do_it(ids, corpusid_fd, vec_fd):
    vec1 = get_vec(int(ids[0]))
    if not vec1 is None: return
    vec2 = [get_vec(int(v)) for v in ids[1:]]
    vec2 = [ v for v in vec2 if not v is None ]
    n = len(vec2)
    if len(vec2) == 0: return
    vec2 = sum(vec2)
    print(ids[0], file=corpusid_fd)
    vec2.reshape(1,-1).tofile(vec_fd)
    if args.flush:
        vec_fd.flush()
        corpusid_fd.flush()

with open(args.output + '.corpusId', 'w') as corpusid_fd:
    with open(args.output + '.vec', 'w') as vec_fd:
        for line in sys.stdin:
            rline = line.rstrip()
            fields = rline.split()
            do_it(fields, corpusid_fd, vec_fd)
