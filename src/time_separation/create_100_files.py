...
import sys
corp_dict = {}
with open('head_corp_by_date_nonullyear_with_int.tsv', 'r') as f:
#with open('corp_by_date_nonullyear_with_int.tsv', 'r') as f:
    for line in f:
        line = line.strip()
        line_arr = line.split('\t')
        corp_dict[line_arr[0]] = int(float(line_arr[3]))
...
final_print = {}
for corpid,dict_val in corp_dict.items():
    for val in lins_arr:
        if not val in final_print:
            final_print[val] = []
        if dict_val <= val:
            final_print[val].append(corpid)  
            #final_print[val].append(corpid+'-'+str(corp_dict[corpid]))
            break

for time_val, corp_ids in final_print.items():
    print(time_val,'\t','\t'.join(corp_ids))
